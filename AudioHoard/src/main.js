...
const isAppDeepLink = value => /^audiohoard:\/\/|^snowify:\/\//i.test(value || '');
...
if (!gotLock) {
  ...
  app.on('second-instance', (_event, argv) => {
    ...
    const deepUrl = argv.find(isAppDeepLink);
...
function handleDeepLink(url) {
  try {
    const cleaned = url.replace(/^(audiohoard|snowify):\/?\/?/i, '');
...
app.whenReady().then(async () => {
  if (app.isPackaged) {
    app.setAsDefaultProtocolClient('audiohoard');
    app.setAsDefaultProtocolClient('snowify');
  } else {
    app.setAsDefaultProtocolClient('audiohoard', process.execPath, [path.resolve(process.argv[1] || '.')]);
  ...
  const coldLink = process.argv.find(isAppDeepLink);
