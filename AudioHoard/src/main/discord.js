  ...
  ipcMain.handle('discord:updatePresence', async (_event, data) => {
    ...
    try {
      await rpcClient.user?.setActivity({
        ...
        largeImageText: data.title || 'AudioHoard',
        smallImageKey: 'logo',
        smallImageText: 'AudioHoard',
        ...
        buttons: [
          { label: 'Get AudioHoard', url: 'https://audiohoard.app' },
          ...(data.videoId ? [{ label: 'Listen on AudioHoard', url: `https://audiohoard.app/track/${data.videoId}` }] : [])
        ],
        instance: false
      });
    } catch (_) {}
