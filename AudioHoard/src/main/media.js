...
function launchHoardApp(appId) {
  const target = hoardApps.find(app => app.id === appId);
...
async function openContentHoard() {
...
function startSharedStateWatcher() {
  ...
  _sharedStateWatchInterval = setInterval(() => {
    try {
      ...
      if (stat.mtimeMs > lastMtime) {
        ...
        if (state && ctx.mainWindow && !ctx.mainWindow.isDestroyed()) {
          ctx.mainWindow.webContents.send('contenthoard:shared-state-updated', state);
        }
      }
    } catch { /* ignore */ }
  }, 2000);
...
process.on('beforeExit', () => {
  if (_sharedStateWatchInterval) clearInterval(_sharedStateWatchInterval);
});
...
function register(ipcMain, ctx) {
  ...
  ipcMain.handle('contenthoard:getApps', () => hoardApps.map(({ id, name }) => ({ id, name })));
  ipcMain.handle('contenthoard:launchApp', (_event, appId) => launchHoardApp(appId));
  ipcMain.handle('contenthoard:open', () => openContentHoard());
  ipcMain.handle('contenthoard:readSharedState', () => readSharedState());
  ipcMain.handle('contenthoard:writeSharedState', (_event, nextState) => writeSharedState(nextState));
  ...
  ipcMain.handle('library:export', async (_event, jsonStr) => {
    const result = await dialog.showSaveDialog(ctx.mainWindow, {
      title: 'Export AudioHoard Library',
      defaultPath: `audiohoard-library-${new Date().toISOString().slice(0, 10)}.json`,
  ...
  ipcMain.handle('library:import', async () => {
    const result = await dialog.showOpenDialog(ctx.mainWindow, {
      title: 'Import AudioHoard Library',
      filters: [{ name: 'JSON', extensions: ['json'] }],
      properties: ['openFile']
    });
    if (result.canceled || !result.filePaths.length) return null;
