...
function register(ipcMain, ctx) {
  ...
  ipcMain.handle('app:getChangelog', async (_event, version) => {
    try {
      ...
      const body = await new Promise((resolve, reject) => {
        const req = https.get(`https://api.github.com/repos/nyakuoff/AudioHoard/releases/tags/${tag}`, { headers: { 'User-Agent': 'AudioHoard', 'Accept': 'application/vnd.github+json' } }, (res) => {
          let data = '';
  ...
  ipcMain.handle('app:getRecentReleases', async () => {
    try {
      const https = require('https');
      const body = await new Promise((resolve, reject) => {
        const req = https.get('https://api.github.com/repos/nyakuoff/AudioHoard/releases?per_page=20', { headers: { 'User-Agent': 'AudioHoard', 'Accept': 'application/vnd.github+json' } }, (res) => {
          let data = '';
          res.on('data', chunk => data += chunk);
          res.on('end', () => res.statusCode === 200 ? resolve(data) : reject(new Error(`GitHub API ${res.statusCode}`)));
        });
