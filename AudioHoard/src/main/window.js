...
function createWindow(ctx) {
  ...
  ctx.mainWindow = new BrowserWindow({
    ...
    icon: nativeImage.createFromPath(path.join(__dirname, '..', '..', '..', 'icons', 'ahicon.ico'))
  });
  ...
  function setupTray() {
    ...
    ctx.tray.setToolTip('AudioHoard');
    ctx.tray.setContextMenu(Menu.buildFromTemplate([
      { label: 'Open AudioHoard', click: () => { ctx.mainWindow?.show(); ctx.mainWindow?.focus(); } },
      { type: 'separator' },
      { label: 'Quit AudioHoard', click: () => { _forceQuit = true; ctx.mainWindow?.close(); } }
    ]));
