...
const { getYtDlpPath, getYtDlpAuthArgs } = require('./ytdlp');
...
function register(ipcMain, ctx) {
  ...
  ipcMain.handle('yt:getStreamUrl', async (_event, videoUrl, quality, trackMeta = {}, sources = ['youtube']) => {
    ...
    function streamErrorMessage(err) {
      const message = String(err?.message || err || 'Unable to resolve stream URL');
      if (/sign in to confirm you.?re not a bot|use --cookies-from-browser|use --cookies/i.test(message)) {
        return 'YouTube is blocking anonymous playback. Set SNOWIFY_YTDLP_COOKIES to a YouTube cookies.txt file, or set SNOWIFY_YTDLP_COOKIES_FROM_BROWSER=chrome/firefox, then restart AudioHoard.';
      }
      return message;
    }
    ...
    for (const source of activeSources) {
      if (source === 'youtube') {
        ...
        try {
          const url = await runYtDlp(['-f', fmt, '--get-url', ...YT_FLAGS, ...authArgs, videoUrl], 15000);
    ...
    return { error: streamErrorMessage(lastError) };
  });
  ...
  ipcMain.handle('yt:downloadAudio', async (_event, videoUrl, quality, videoId) => {
    ...
    const authArgs = await getYtDlpAuthArgs();
    return new Promise((resolve, reject) => {
      const proc = execFile(getYtDlpPath(), ['-f', fmt, '-o', path.join(cacheDir, videoId + '.%(ext)s'), '--no-part', '--no-warnings', '--no-playlist', '--no-check-certificates', ...authArgs, videoUrl], { timeout: 120000 }, (err, _stdout, stderr) => {
  ...
  ipcMain.handle('song:saveTo', async (_event, videoUrl, title, artist) => {
    ...
    return new Promise((resolve) => {
      const proc = execFile(getYtDlpPath(), ['-x', '--audio-format', 'mp3', '--audio-quality', '0', '--no-part', '--no-warnings', '--no-playlist', '--no-check-certificates', ...authArgs, '-o', result.filePath, videoUrl], { timeout: 300000 }, (err, _stdout, stderr) => {
  ...
  ipcMain.handle('yt:getVideoStreamUrl', async (_event, videoId, quality, premuxed) => {
    ...
    return new Promise((resolve, reject) => {
      execFile(getYtDlpPath(), ['-f', fmt, '--get-url', '--no-warnings', '--no-playlist', '--no-check-certificates', ...authArgs, `https://music.youtube.com/watch?v=${videoId}`], { timeout: 20000 }, (err, stdout, stderr) => {
