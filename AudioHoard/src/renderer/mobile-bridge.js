var __AudioHoardMobile = (() => {
  ...
  async function getLyrics(trackName, artistName, albumName, durationSec) {
    ...
    try {
      ...
      const resp = await fetch(`${LRCLIB}/get?${params.toString()}`, {
        headers: { "Lrclib-Client": "AudioHoard Mobile" }
      });
      if (!resp.ok) {
        cacheSet(key, null);
        return null;
  ...
  async function exportLibrary(jsonStr) {
    ...
    a.href = url;
    a.download = `audiohoard-library-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`;
  ...
  function installMobileBridge() {
    ...
    window.snowify = {
      ...
      resolveImageUrl,
      // YouTube Music
      search: async (q, musicOnly) => proxifyArtworkUrls(await search(q, musicOnly)),
      searchArtists: async (q) => proxifyArtworkUrls(await searchArtists(q)),
      searchAlbums: async (q) => proxifyArtworkUrls(await searchAlbums(q)),
      searchVideos: async (q) => proxifyArtworkUrls(await searchVideos(q)),
      searchPlaylists: async (q) => proxifyArtworkUrls(await searchPlaylists(q)),
      getPlaylistVideos: async (id) => proxifyArtworkUrls(await getPlaylistVideos(id)),
      searchSuggestions: async (q) => proxifyArtworkUrls(await searchSuggestions(q)),
