...
import { escapeHtml, addScrollArrows, formatFollowers, showToast, renderArtistLinks, buildAudioHoardLink } from './utils.js';
...
export async function openArtistPage(artistId) {
  ...
  followBtn.onclick = () => {
    ...
    import('./library.js').then(m => {
      m.renderSidebarArtists();
      if (state.currentView === 'library') m.renderLibrary();
    });
  };
