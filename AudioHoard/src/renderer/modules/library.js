...
import { escapeHtml, showToast, showInputModal, resolveImageUrl, buildAudioHoardLink } from './utils.js';
  ...
  menu.addEventListener('click', async (ev) => {
    ...
    switch (action) {
      ...
      case 'move-down':
        ...
        break;
      case 'share':
        navigator.clipboard.writeText(buildAudioHoardLink('track', track.id));
...
export function renderLibrary() {
  ...
  const hasPlaylistContent = allPlaylists.some(p => p.tracks.length) || state.playlists.length > 0;
  const hasFollowedArtists = state.followedArtists.length > 0;

  if (!hasPlaylistContent && !hasFollowedArtists) {
    container.innerHTML = `
      <div class="empty-state">
  ...
  const playlistGrid = hasPlaylistContent ? `
    <section class="library-section">
      <h3 class="library-section-title">${I18n.t('library.playlists')}</h3>
      <div class="library-grid">${allPlaylists.map(p => {
  ...
  const artistGrid = hasFollowedArtists ? `
    <section class="library-section">
      <h3 class="library-section-title">${I18n.t('library.followedArtists')}</h3>
      <div class="library-grid library-artist-grid">
        ${state.followedArtists.map(a => `
          <div class="lib-card lib-artist-card" data-artist-id="${escapeHtml(a.artistId)}">
            <div class="lib-card-cover lib-artist-cover">
              ${a.avatar
                ? `<img src="${escapeHtml(a.avatar)}" alt="" />`
                : `<svg width="54" height="54" viewBox="0 0 24 24" fill="currentColor"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/></svg>`}
            </div>
            <div class="lib-card-name">${escapeHtml(a.name)}</div>
            <div class="lib-card-meta">${I18n.t('artist.type')}</div>
          </div>`).join('')}
      </div>
    </section>` : '';

  container.innerHTML = playlistGrid + artistGrid;
