...
export default function MoviesPage() {
  ...
  const movieCatalogs = getVisibleLists("movies");
  ...
  const lastSectionRef = useCallback((node: HTMLDivElement | null) => {
    ...
    observerRef.current = new IntersectionObserver(entries => {
      if (entries[0].isIntersecting && visibleCount < movieCatalogs.length) {
        setVisibleCount(prev => Math.min(prev + 3, movieCatalogs.length));
      }
    }, { threshold: 0.1 });

    if (node) observerRef.current.observe(node);
  }, [visibleCount, movieCatalogs.length]);
      ...
      <div className="max-w-[1440px] mx-auto w-full flex-1 flex flex-col">
        {movieCatalogs.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-12 mt-20 border border-white/5 rounded-3xl bg-surface-container-lowest/50">
            <span className="material-symbols-outlined text-6xl text-white/10 mb-4">movie</span>
            <h3 className="text-headline-md text-white/80 mb-2">No movie lists available</h3>
            <p className="text-body-md text-white/40 mb-6">Install an addon with movie catalogs or manage your lists to see content here.</p>
          ...
          <div className="flex flex-col gap-8">
            <div className="px-6 mb-4 animate-in fade-in slide-in-from-left-4 duration-700">
              <h1 className="text-display-md text-white">Movies</h1>
            </div>
            
            <AnimatePresence mode="popLayout">
              {movieCatalogs.slice(0, visibleCount).map((cat, i) => (
                <motion.div 
                  key={`${cat.id}-${i}`}
                  initial={{ opacity: 0, y: 30 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.6, delay: i < 3 ? i * 0.1 : 0 }}
                  ref={i === Math.min(visibleCount, movieCatalogs.length) - 1 ? lastSectionRef : null}
                >
                  <CatalogSection 
                    title={cat.name}
