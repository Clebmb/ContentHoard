  ...
  public static async spawn(
    ...
    if (app.isPackaged) {
      ...
      if (!fs.existsSync(binaryPath)) {
        dialog.showErrorBox(
          "Fatal",
          "PlayHoard Python instance binary not found. Please check if it has been removed by Windows Defender."
        );

        app.quit();
        throw new Error(`PlayHoard Python RPC binary not found at ${binaryPath}`);
      }
