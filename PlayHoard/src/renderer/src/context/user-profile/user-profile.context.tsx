import { darkenColor, resolveImageUrl } from "@renderer/helpers";
...
export function UserProfileContextProvider({
  ...
  const getHeroBackgroundFromImageUrl = async (imageUrl: string) => {
    const output = await average(resolveImageUrl(imageUrl) ?? imageUrl, {
      amount: 1,
      format: "hex",
    });
  ...
  const getBackgroundImageUrl = () => {
    if (selectedBackgroundImage && isMe)
      return resolveImageUrl(selectedBackgroundImage) ?? "";
    if (userProfile?.backgroundImageUrl)
      return resolveImageUrl(userProfile.backgroundImageUrl) ?? "";
  ...
  const getUserProfile = useCallback(async () => {
    if (userDetails?.id === userId) {
      ...
      if (userDetails.profileImageUrl) {
        getHeroBackgroundFromImageUrl(userDetails.profileImageUrl).then(
  ...
  const getBadges = useCallback(async () => {
    ...
    const badges = await window.electron.hydraApi
      .get<Badge[]>(`/badges?${params.toString()}`, { needsAuth: false })
      .catch(() => []);
    setBadges(badges);
  }, [i18n]);
